	...
	<div
		...
		<el-row>
          ...
          <div style="text-align: center;margin-bottom: 10px;">
            <span style="font-size: 28px;font-weight: 700;">待恢复图片</span>
          </div>
          <el-image
            class="image"
				    ref="preview"
              ...
              <el-button type="primary" class="button" @click="uploadImage()" round>上传图片</el-button>
			        <el-button type="primary" class="button" @click="restorePicture()" round>恢复图片</el-button>
            </div>
			    </div>
			  </el-card>
		  </el-col>
...
<!-- 		    <div >
		     ...
		     <i class="el-icon-right"></i>
		      <el-steps direction="vertical" :active="stepflag" finish-status="success" simple >
          ...
          <div style="text-align: center;margin-bottom: 10px;">
            <span style="font-size: 28px;font-weight: 700;">已恢复图片</span>
          </div>
			    <el-image
			    	class="image"
            ref="preview"
            :src="restore_image_src[0]"
            :preview-src-list="restore_image_src">
          </el-image>
			    <div style="padding: 14px;">
			      <div style="text-align: center;">
			        <el-button type="primary" class="button" round @click="preserveRestoreImage()">保存图片</el-button>
			      </div>
			    </div>
			  </el-card>
		  </el-col>
    ...
    <!-- 初代代码，保留待定
		...
		<el-dialog
		  ...
		  <div class="demo-image">
		    <div class="block" v-for="i in Math.ceil(restorePictureurls.length/6)" :key="i">
			  <el-row>
			    <el-col v-if="(i-1)*6+j<=restorePictureurls.length" :span="4" v-for="j in 6" :key="j">
					<el-card  shadow="hover" :class="[restorePictureischoose[(i-1)*6+j] ? 'selectedcard' : 'card']" >
					  <img
						  :src="getpath(restorePictureurls[(i-1)*6+j-1],'restore/')"
						  class="image"
						  @click="SelectrestorePicture((i-1)*6+j)"
					  />
					</el-card>
				</el-col>
			  </el-row>
  ...
  data() {
    return {
      ...
      pending_image_src:[//待处理图片（原图）文件src
        this.$global.interfaceSetting.restorePicture.pendingImage,
      ],
      restore_image_src:[//已进行修复后图片文件src
        this.$global.interfaceSetting.restorePicture.restoreImage,
      ],
      restore_image_type:".png",//已恢复图片类型
      ...
      /* 初代代码，保留待定
        ...
        restorePictureischoose:[],//记录原图是否被选中以改变样式
        restorePictureurls:[],//待检测图片集
        restorePictureIndex:null,//待检测图片路径或编号
  ...
  */
  methods:{
    ...
    restorePicture(){
      ...
      //配置axios url以及参数
      const target_url = `http://localhost:5000/RestorePicture`;
      ...
      .then((res) => {
        ...
        //取得后端目标图片文件
        axios.get(restore_image.url, restore_image).then((ans) => {
          //更新待处理图片src
          this.restore_image_src = [];
          this.restore_image_src[0] = this.getObjectURL(ans.data);
          //更新嵌入水印图片文件类型
          if(ans.data.type != null){
            this.restore_image_type = ans.data.type;
          }else this.restore_image_type = ".png";
        });
    ...
    //下载保存恢复后的图片文件
    preserveRestoreImage(){
      //将图片的src属性作为URL地址
      var a = document.createElement('a')
      var event = new MouseEvent('click')
      a.download = "new_image" + this.restore_image_type //'保存图片名称'
      a.href = this.restore_image_src[0];
    ...
    chooseFileChanged(){
      try{
        ...
        new Promise((resolve,reject)=>{
          ...
          //更新嵌入水印图片，置为初始状态
          this.restore_image_src = [];
          this.restore_image_src[0] = this.$global.interfaceSetting.restorePicture.restoreImage;
        ...
        // 定义 filereader对象
          ...
          reader.onload = e=>{
              ...
              image.onload = _=>{
                ...
                _this.restorePictureIndex=fileObj.name;
    ...
    // 基于准备好的dom，初始化echarts实例
	  ...
	  InitDate(){
		  ...
		  files.keys().forEach(path=> {
				...
				let newpath=paths[1]
				this.restorePictureurls[i]=newpath;
				i++;
		  })
		  for(let i=0;i<=this.restorePictureurls.length;i++){
		  		this.restorePictureischoose[i]=false;
		  }
		  // console.log(this.restorePictureurls);
		  // console.log(this.restorePictureischoose);
	  },
    //选择原图
	  SelectrestorePicture(flag){
      this.stepflag=1;
		  if(this.CurrentSelectedPicture!=null){
		  	this.restorePictureischoose[this.CurrentSelectedPicture]=false;
		  }
		  this.restorePictureischoose[flag]=true;
		  this.CurrentSelectedPicture=flag;
		  this.restorePictureIndex=this.restorePictureurls[flag-1];
      this.srcList[0][0]=this.getpath(this.restorePictureIndex,"tampered/")
		  //起刷新效果、原因未知。。。
	  ...
	  RestorePicture(){
	  	this.dialogVisible1 = false;
      if(this.restorePictureIndex==null){
        this.$message({ //成功修改提示
          message: '请上传待恢复图片!!',
          type: 'error'
        })
      ...
      let restorePicturepath={
        path:'D:/2023-2-6/image/tampered/'+this.restorePictureIndex
      };
      const path = `http://localhost:5000/RestorePicture`;
	  	axios.post(path,restorePicturepath)
	  	  .then((res) => {
          this.stepflag=2;
	  	    this.$message({ //成功修改提示
	  	      message: '恢复成功!!',
...
<style scoped>
  ...
  .RestorePicture /deep/ .el-upload-dragger{
  			width: 650%;
  			/* height: 30%; */
  	    margin-top: 10%;
  	    margin-left: 1%;
  ...
  .RestorePicture /deep/ .el-card__body {
      padding: 0px;
  }
  .card{
	  padding: 10px;
  }
  .RestorePicture /deep/ .el-icon-loading{
	  font-size: 70px;
  }
  .RestorePicture /deep/ .rightel-card {
  ...
  .RestorePicture /deep/ .leftel-card {
      border: 1px solid #EBEEF5;
      background-color: #FFF;
      color: #303133;
      -webkit-transition: .3s;
